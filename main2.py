...
def redo_hotel_tripadvisor_reviews(path, driver):
    ...
    result.update({"reviewer_info": lines3})
    save_data(path, result)
def get_hotel_reviewer_data():
    paths=[]
    for i in xrange(19):
        temp_path = 'data/split/Tripadvisor_SF_sub_' + str(index) + '.xlsx'
    ...
    for path in paths:
        ...
        reviewers_id_list.append([line[1] for line in reviewers[1:]])
    return reviewers_id_list
...
def update_hotel_reviewer(path,driver):
    ...
    #reviewers_id_set =[]#get_hotel_reviewer_data()
    reviewers_info=[]
    ...
    result.update({"review_info":reviews})
    save_data(path,result)
def update_hotel_reviewer_score(path,driver):
    # driver = webdriver.Chrome()
    result = OrderedDict()
    xls = get_data(path)
    data=json.dumps(xls, default=json_serial)
    ...
    for index,reviewer in enumerate(reviewers[1:]):
        try:
            ...
            while len(reviewer)<18:
                reviewer.append()
            print not reviewer[11],reviewer[11]
            if not reviewer[11] and reviewer[11]!=0 and reviewer[11]!='0':
                link=reviewer[2]
                # print reviewer
                print link
                ...
                reviewers[index][11]=scores[0]
                reviewers[index][12]=scores[1]
                reviewers[index][13]=scores[2]
                reviewers[index][14]=scores[3]
                reviewers[index][15]=scores[4]
        except:
            pass
    result.update({"hotel_info":lines})
    result.update({"reviewer_info":reviewers})
...
if __name__ == '__main__':
    ...
    for i in xrange(1, 41):
            path = 'data/sub/Tripadvisor_SF_sub_' + str(index) + '_' + str(i) + '.xlsx'
            print path
        # try:
            ...
            try:
                # crawling reviewer
                print '===update_hotel_reviewer:'+str(index)+'_'+str(i)
                update_hotel_reviewer(path,driver)

                print '===update_hotel_reviewer_score'+str(index)+'_'+str(i)
                update_hotel_reviewer_score(path,driver)
            except:
                pass
            driver.close()
